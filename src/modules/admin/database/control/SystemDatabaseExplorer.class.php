...
class SystemDatabaseExplorer extends TPage
{
    ...
    public function __construct()
    {
        ...
        // create action
        $action1 = new TDataGridAction(urlRoute('/admin/system/tablelist/load'));
        $action1->setParameter('register_state', 'false');
        $action1->setImage('fa:table');
        $action1->setField('database');
        $action1->setLabel('View');
        
        $action2 = new TDataGridAction(urlRoute('/admin/system/database/explorer/export/csv'));
        ...
        $action3 = new TDataGridAction(urlRoute('/admin/system/database/explorer/export/sql'));
        ...
        $vbox->add(new TXMLBreadCrumb('menu.xml', '/admin/system/database/explorer'));
        $vbox->add($html);
    ...
    public static function onExportCSV($param)
    {
        try
        {
            ...
            if ($tables)
            {
                ...
                $zip->close();
                parent::openFile(urlRoute('download.php?file='.$output));
            }
            TTransaction::close();
        }
        catch (Exception $e)
