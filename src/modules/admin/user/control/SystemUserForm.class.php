...
class SystemUserForm extends TPage
{
    ...
    public function __construct()
    {
        ...
        $btn = $this->form->addAction(_t('Save'), new Action(route('/admin/system/user/save'), 'POST'), 'fa:floppy-o');
        $btn->class = 'btn btn-sm btn-primary';
        $this->form->addActionLink(_t('Clear'), new Action(urlRoute('/admin/system/user/clear'), 'POST'), 'fa:eraser red');
        $this->form->addActionLink(_t('Back'), new Action(urlRoute('/admin/system/user/reload')), 'fa:arrow-circle-o-left blue');
    ...
    public static function onSave($param)
    {
        try {
            ...
            if (!empty($param['id'])) {
                $object = SystemUser::find($param['id']);
                $object->active = 'Y';
            } else {
            ...
            $object->login = $param['login'];
            if (!empty($param['password'])) {
                $object->password = password_hash($param['password'], PASSWORD_BCRYPT);
            }
            $object->email = $param['email'];
            $object->frontpage_id = $param['frontpage_id'];
            $object->system_unit_id = $param['system_unit_id'];
            ...
            Redirect::ajaxLoadPage(urlRoute('/admin/system/user/edit'), ['key' => $object->id, 'id' => $object->id]);
     ...
     * @throws Exception
     */
    private static function validate(array $param)
    {
        ...
        if (empty($param['id'])) {
            ...
            if (!empty($param['password']) and $param['password'] !== $param['repassword']) {
